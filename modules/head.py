#!/usr/bin/env python
"""
head.py - Torp HTTP Metadata Utilities
...
def head(torp, input): 
   ...
   if not uri and hasattr(torp, 'last_seen_uri'): 
      try: uri = torp.last_seen_uri[input.sender]
      except KeyError: return torp.say('?')
   ...
   except IOError: return torp.say("Can't connect to %s" % uri)
   except httplib.InvalidURL: return torp.say("Not a valid URI, sorry.")

   if not isinstance(info, list): 
      try: info = dict(info)
      except TypeError: 
         return torp.reply('Try .head http://example.org/ [optional header]')
      info['Status'] = '200'
   else: 
      newInfo = dict(info[0])
   ...
   if header is None: 
      ...
      torp.reply(', '.join(data))
   else: 
      headerlower = header.lower()
      if info.has_key(headerlower): 
         torp.say(header + ': ' + info.get(headerlower))
      else: 
