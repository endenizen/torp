...
# coding=utf-8
"""
translate.py - Torp Translation Module
...
def tr(torp, context): 
   ...
   if input != output: 
      ...
      torp.reply(msg)
   else: torp.reply('Language guessing failed, so try suggesting one!')

tr.rule = ('$nick', ur'(?:([a-z]{2}) +)?(?:([a-z]{2}) +)?["“](.+?)["”]\? *$')
tr.example = '$nickname: "mon chien"? or $nickname: fr "mon chien"?'
tr.priority = 'low'

def mangle(torp, input): 
   ...
   torp.reply(phrase or 'ERRORS SRY')
