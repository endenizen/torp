#!/usr/bin/env python
"""
codepoints.py - Torp Codepoints Module
...
def u(torp, input): 
   ...
   arg = input.bytes[3:]
   # torp.msg('#inamidst', '%r' % arg)
   if not arg: 
      return torp.reply('You gave me zero length input.')
   elif not arg.strip(' '): 
      if len(arg) > 1: return torp.reply('%s SPACEs (U+0020)' % len(arg))
      return torp.reply('1 SPACE (U+0020)')
   ...
   if printable: 
      ...
      if extended: 
         ...
         for i, result in enumerate(results): 
            if (i < 2) or ((i == 2) and (len(results) < 4)): 
               ...
               torp.say(result + ' [...]')
         if not results: 
            torp.reply('Sorry, no results')
      else: 
         # look up a codepoint freely
         result = codepoint_simple(arg)
         if result is not None: 
            torp.say(result)
         else: torp.reply("Sorry, no results for %r." % arg)
   else: 
      ...
      # look up more than three podecoints
         ...
         torp.reply(' '.join('U+%04X' % ord(c) for c in text))
      else: torp.reply('Sorry, your input is too long!')
u.commands = ['u']
u.example = '.u 203D'

def bytes(torp, input): 
   """Show the input as pretty printed bytes."""
   b = input.bytes
   torp.reply('%r' % b[b.find(' ') + 1:])
