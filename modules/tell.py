#!/usr/bin/env python
"""
tell.py - Torp Tell and Ask Module
...
def setup(self): 
   ...
   self.tell_filename = os.path.join(os.path.expanduser('~/.torp'), fn)
...
def f_remind(torp, input): 
   ...
   if not os.path.exists(torp.tell_filename): 
      return

   if len(tellee) > 20: 
      return torp.reply('That nickname is too long.')

   timenow = time.strftime('%d %b %H:%MZ', time.gmtime())
   if not tellee in (teller.lower(), torp.nick, 'me'): # @@
      # @@ <deltab> and year, if necessary
      warn = False
      if not torp.reminders.has_key(tellee): 
         torp.reminders[tellee] = [(teller, verb, timenow, msg)]
      else: 
         # if len(torp.reminders[tellee]) >= maximum: 
         #    warn = True
         torp.reminders[tellee].append((teller, verb, timenow, msg))
      ...
      torp.say('You can %s yourself that.' % verb)
   else: torp.say("Hey, I'm not as stupid as Monty you know!")

   dumpReminders(torp.tell_filename, torp.reminders) # @@ tell
f_remind.rule = ('$nick', ['tell', 'ask'], r'(\S+) (.*)')

def getReminders(torp, channel, key, tellee): 
   lines = []
   template = "%s: %s <%s> %s %s %s"
   today = time.strftime('%d %b', time.gmtime())

   for (teller, verb, datetime, msg) in torp.reminders[key]: 
      if datetime.startswith(today): 
         datetime = datetime[len(today)+1:]
      lines.append(template % (tellee, datetime, teller, verb, tellee, msg))

   try: del torp.reminders[key]
   except KeyError: torp.msg(channel, 'Er...')
   return lines

def message(torp, input): 
   ...
   reminders = []
   remkeys = list(reversed(sorted(torp.reminders.keys())))
   for remkey in remkeys: 
      if not remkey.endswith('*') or remkey.endswith(':'): 
         if tellee.lower() == remkey: 
            reminders.extend(getReminders(torp, channel, remkey, tellee))
   ...
   if reminders[maximum:]: 
      torp.say('Further messages sent privately')
      for line in reminders[maximum:]: 
         torp.msg(tellee, line)

   if len(torp.reminders.keys()) != remkeys: 
      dumpReminders(torp.tell_filename, torp.reminders) # @@ tell
message.rule = r'(.*)'
