...
def service(torp, input, command, args): 
   ...
   if not 'text/plain' in info.get('content-type', '').lower(): 
      return torp.reply("Sorry, the service didn't respond in plain text.")
   bytes = web.get(uri)
   lines = bytes.splitlines()
   if not lines: 
      return torp.reply("Sorry, the service didn't respond any output.")
   torp.say(lines[0][:350])

def refresh(torp): 
   if hasattr(torp.config, 'services'): 
      services = torp.config.services
...
def o(torp, input): 
   ...
   if (not o.services) or (text == 'refresh'): 
      length, added = refresh(torp)
      if text == 'refresh': 
         msg = 'Okay, found %s services.' % length
         if added: 
            msg += ' Added: ' + ', '.join(sorted(added)[:5])
            if len(added) > 5: msg += ', &c.'
         return torp.reply(msg)

   if not text: 
      return torp.reply('Try %s for details.' % o.serviceURI)
   ...
   if not o.services.has_key(command): 
      return torp.reply('Sorry, no such service. See %s' % o.serviceURI)

   if hasattr(torp.config, 'external'): 
      default = torp.config.external.get('*')
      manifest = torp.config.external.get(input.sender, default)
      if manifest: 
         commands = set(manifest)
         if (command not in commands) and (manifest[0] != '!'): 
            return torp.reply('Sorry, %s is not whitelisted' % command)
         elif (command in commands) and (manifest[0] == '!'): 
            return torp.reply('Sorry, %s is blacklisted' % command)
   service(torp, input, command, args)
o.commands = ['o']
o.example = '.o servicename arg1 arg2 arg3'
o.services = {}
o.serviceURI = None

def snippet(torp, input): 
   if not o.services: 
      refresh(torp)
