...
def yi(torp, input): 
   ...
   if extraraels == 4: 
      return torp.say('Yes! PARTAI!')
   else: torp.say('Not yet...')
yi.commands = ['yi']
yi.priority = 'low'
...
def tock(torp, input): 
   ...
   info = u.info()
   u.close()
   torp.say('"' + info['Date'] + '" - tycho.usno.navy.mil')
tock.commands = ['tock']
tock.priority = 'high'
...
def npl(torp, input): 
   ...
   if data: 
      ...
      torp.say(result + ' - ntp1.npl.co.uk')
   else: torp.say('No data received, sorry')
npl.commands = ['npl']
npl.priority = 'high'
